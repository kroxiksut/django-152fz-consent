...
## Авторизованный и неавторизованный слой

- В модуле есть сценарии для авторизованного субъекта (`user`) и для неавторизованного субъекта через `anonymous_token`.
- Для форм и публичных страниц допускается неавторизованный поток, если это разрешено политикой цели и настройками самообслуживания.
- Для внутренних и защищённых операций проект обычно использует авторизованный слой и собственные проверки доступа.
...
- При расширении API важно явно фиксировать, какие методы предназначены для авторизованного слоя, а какие допускают работу без авторизации.

## Навигация по разделу

- Предыдущий: [Настройки и контракт политик](./configuration.md)
- Следующий: [Тестирование модуля согласий](./testing.md)
