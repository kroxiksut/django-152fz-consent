# Документы для ИИ: модуль согласий

Этот раздел содержит материалы для ИИ-агентов по интеграции модуля согласий.

Сами инструкции поддерживаются только на английском языке:

- [README.md](./README.md)
- [integration.md](./integration.md)
- [forms.md](./forms.md)
- [verified-consents.md](./verified-consents.md)
- [goskey-future.md](./goskey-future.md)

Примечание по quality-gate: основной инструмент статической типизации в репозитории — `pyright`.
